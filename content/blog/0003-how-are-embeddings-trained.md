...
{{ manim(file="0003-the-exam-and-its-temperature", title="A query vector on the unit circle with one match and five decoys, graded by a softmax whose temperature is swept up and down", step_one="The bars are the softmax over the six candidates and the loss is just the match's share, so the whole objective is one multiple-choice question about angles.", step_two="Watch the bars as the temperature moves: at tiny tau only the match and the one near-miss keep any weight at all, and at large tau every candidate is a little bit wrong and none of them in particular.", step_three="The arrows are the actual gradient, drawn with length proportional to each candidate's softmax weight — when descent runs, the near-miss swings nearly fifty degrees around the ring while the decoys on the far side move by a tenth of a degree or less.", caption="Temperature decides which mistakes count. The gradient on each negative is scaled by how confusable it already was, so near-misses do essentially all the teaching and comfortable decoys contribute nothing.") }}
...
{{ manim(file="0003-alignment-and-uniformity", title="Six coloured pairs of points on a circle, pulled together by an alignment term and pushed apart by a uniformity term", step_one="With only alignment running, every pair closes its chord and the alignment number falls to zero — but the pairs stay bunched where they started and the uniformity number gets worse.", step_two="Then the whole map slides onto a single point. Alignment still reads zero: the objective genuinely cannot tell this collapse apart from the good solution, which is why the second term has to exist.", step_three="Uniformity alone spreads the points evenly and shreds the pairs, with chords crisscrossing the circle. Only with both terms do the couples merge and land evenly spaced.", caption="Alignment alone is satisfied by a constant map, and uniformity alone destroys the pairs. The geometry you actually want only appears as the equilibrium between the two, on the sphere the objective is implicitly sculpting.") }}
...
{{ manim(file="0003-rotation-is-invisible", title="Four embeddings on a circle being rotated while a table of pairwise cosine similarities is recomputed from their live coordinates", step_one="Both columns are computed from the points on screen: six pairwise cosines on top, the raw coordinates of a underneath.", step_two="As the map turns, the coordinate readout scrambles through every value it can take while not one digit of the similarity table changes.", step_three="Two runs of the same recipe end up as congruent maps at different orientations: cos(a, b) agrees to three decimals, and a's coordinates do not even agree on a sign.", caption="The loss is a function of dot products alone, so every rotation of a solution is another, equally optimal solution. Nothing in training picks which way is up — which is why coordinates from two runs are never comparable, even when the geometry is identical.") }}
