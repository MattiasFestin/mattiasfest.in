...
{{ manim(file="0006-confidently-right-gets-fined", title="Nine emails fitted by least squares, then three obvious spam emails added and the line refitted", step_one="Six ham at 0, three spam at 1, a least-squares line through them and a 0.5 cutoff: the boundary lands at x = 4.1 and all nine emails sit on the right side of it.", step_two="Zoom out and add the three caricature spam emails at 40, 50 and 60 trigger words. Out there the old line is not wrong, it predicts 9.10 against a label of 1 — and squared error bills it 65.7 for the overshoot.", step_three="So the fit flattens to hush that bill, total squared error falling from 134.1 to 1.7, and back on the scale a 0/1 label lives on the boundary has slid out to x = 14.8: the three original spam emails now score below the cutoff and sail through as ham.", caption="Both fits are solved on screen from the points on screen, and the live loss readout during the refit is evaluated on all twelve of them. Adding the most unambiguously correct data in the set made the classifier worse, because least squares is minimising distances when you wanted it to minimise mistakes.") }}
...
{{ manim(file="0006-the-gradient-dies-where-it-hurts", title="A sigmoid with a score sliding along it, beside the gradient magnitude of squared error and of cross-entropy plotted against that same score", step_one="Squashing the score fixes the overshoot, and the chain rule on squared error picks up the slope of the squashing function as a factor — a factor that is near zero at both flat ends.", step_two="Slide the score from confidently right to confidently wrong. How wrong the model is climbs to 0.998 while the push collapses to 0.0049; it had already peaked back at s = -0.7, where the model was barely wrong at all.", step_three="The cross-entropy curve carries no such factor. At the same score it pushes 203 times harder, and it is still at full strength exactly where the model is most wrong.", caption="Both curves are the exact derivatives with respect to the score, evaluated on the sigmoid drawn beside them. Squared error plus squashing is a loss that stops teaching precisely when there is the most to learn.") }}
...
{{ manim(file="0006-one-scorer-many-products", title="A thousand scored emails under a sliding cutoff, with live accuracy, precision and recall, and the precision-recall curve the sweep traces", step_one="990 ham and 10 spam. The always-ham model posts 0.990 accuracy while catching nothing whatsoever, which is the entire case against reporting accuracy on an imbalanced problem.", step_two="One real scorer and one cutoff sliding down through it. Nothing about the model changes; the counts underneath it do, and precision and recall trade places the whole way down.", step_three="Sweep every cutoff and the same scorer traces a whole product line: at 2.0 it catches 6 of the 10 spam and buries 17 real emails, at 0.5 it catches all 10 and buries 287.", caption="Every count is counted from the thousand scored emails the histogram is drawn from. Same weights, same scores, opposite souls — and where you draw the line prices the two ways of being wrong, which is a decision the mathematics never makes for you.") }}
