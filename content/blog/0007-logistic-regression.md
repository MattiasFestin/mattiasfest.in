...
{{ manim(file="0007-sigmoid-from-log-odds", title="The score's log-odds declaration solved for a probability, then read live off the curve", step_one="Declare that the score IS the log-odds, log(p/(1-p)) = s, and solve for p: the sigmoid falls out of the algebra rather than being picked from a catalogue of S-shaped curves.", step_two="Slide the score up to the boss's 2.3 and read the card live: odds of 9.97-to-1, sigma(2.3) = 0.909 — 91% spam, by unit conversion.", step_three="Flip the sign and the curve mirrors around the coin flip at 0.5; push the score to +/-6 and it saturates, pinning within a quarter of a percent of 0 or 1.", caption="Every number in the card is sigma evaluated at the score the dot is sitting on, read straight off the curve underneath it — nothing is looked up from a table.") }}
...
{{ manim(file="0007-the-punchline-gradient", title="Cross-entropy's gradient collapsing to Part 1's, then a real training run showing the valley and its missing floor", step_one="Part 2's squared-error gradient carried a sigma-prime factor that strangled it. Cross-entropy's chain rule picks up the same sigma-prime, and it cancels clean against the log's denominators, leaving sigma(s) - y: not suppressed, algebraically annihilated.", step_two="Real gradient descent on the two Gaussian clusters from the convexity snippet: the entire update is g = p - y, and the loss slides from 0.693 to 0.110 monotonically — one valley, downhill is always the right direction.", step_three="Four separable points, no regularization: the fitted sigmoid steepens without bound as w climbs from 0.38 to 8.52 and the loss creeps toward zero, never arriving. Add ridge and w settles at 2.90 and stops moving.", caption="The derivation panels are the post's algebra verbatim, the classification run is real gradient descent trained here from scratch, and the separable-data curves are the post's own four-point example, with and without ridge.") }}
...
{{ manim(file="0007-softmax-goes-multiclass", title="Four score lines becoming four probabilities, then the algebra and a numeric sweep showing sigmoid IS 2-class softmax", step_one="Every class gets its own line: four unbounded scores, exponentiated and normalized, become four probabilities that sum to exactly one, the vote generalized from one boundary to K of them.", step_two="Pin the second class's score at zero and the softmax ratio simplifies term by term to 1 over 1 plus e to the minus s: sigmoid isn't a separate function, it's softmax with two classes, one of them held at zero.", step_three="Sweep the score and compare sigma(s) against 2-class softmax([s, 0])[0] side by side: the two readouts agree to eight decimal places at every score, because they are the same computation.", caption="The bar heights are the post's own four-class demo scores, and the closing sweep evaluates both formulas live rather than asserting they match.") }}
